...
tags: ["model", "google", "multimodal", "gemini", "code-execution", "llm", "speed", "reasoning", "agent", "大语言模型", "轻量化", "高性价比"]
related: [gemini, ssi-bench, glm-4.6v, 空间智能, google-deepmind, agentic-vision, 思考-行动-观察, chain-of-thought, pinchbench, minimax-m2.1, kimi-k2.5, fusion-多模型融合方案, openrouter]
created: 2026-02-25
updated: 2026-06-14
sources: ["gemini-3仅得33.6分！清华发布首个「约束流形」空间智能基准.md", "gemini-3「开眼」像素级操控！谷歌回应deepseek-ocr2.md", "谷歌看了都沉默：自家「黑科技」火了，但为啥研发团队一无所知？.md", "龙虾最佳适配模型，openclaw之父给出了推荐.md", "rss/平替版fable-5的打开教程来了：性能追平，成本砍半.md"]
---

# Gemini-3-Flash

[[Gemini-3-Flash]] 是 [[Google DeepMind]] 开发的 [[Gemini]] 系列多模态大模型，定位为系列中的**轻量化与高性价比**版本。该模型引入了 [[agentic-vision]]（智能体视觉）功能，实现了视觉理解能力的显著进化，同时也是在 [[清华大学]] 发布的 [[SSI-Bench]]（约束流形空间智能基准）测试中表现最好的闭源模型。此外，在 [[PinchBench]] 基准测试中，该模型以 95.1% 的成功率排名第一，是执行复杂 Agent 任务的热门选择。

## 定价与多模型融合

Gemini 3 Flash 的调用成本极为低廉，每百万输入约 0.5 美元，输出约 3 美元。

在 [[OpenRouter]] 的 [[fusion-多模型融合方案|Fusion 融合实验]]中，Gemini 3 Flash 与 [[kimi-k2-6|Kimi K2.6]] 及 [[deepseek-v4-pro|DeepSeek V4 Pro]] 组成了平价融合阵容，在 [[draco-深度研究基准测试集|DRACO]] 深度研究基准测试中，以极低成本实现了追平旗舰模型 [[claude-fable-5|Claude Fable 5]] 的综合表现。
