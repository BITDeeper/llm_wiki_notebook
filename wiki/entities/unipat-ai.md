...
type: entity
title: UniPat AI
tags: ["organization", "ai", "forecasting", "ai-research", "startup", "vision", "ai-lab", "open-source", "research", "研究机构", "评测", "ai-company", "prediction-infrastructure", "echo", "multimodal", "vision-agent", "ai-research-lab", "medical-ai", "ai-evaluation", "agent", "ai评测", "组织"]
related: ["echoz-1-0", "echo-system", "train-on-future", "swe-vision", "babyvision", "uniscientist", "进化式多学科合成", "红杉中国-xbench-团队", "echoz-1.0", "automated-rubric-search", "point-aligned-elo", "map-reduce-agent", "saas-bench", "computer-use-agent", "任务执行范式"]
created: 2026-03-30
updated: 2026-05-25
sources: ["黑马ai横扫预测市场！预测未来胜率已超过人类.md", "500行极简开源框架，硬刚gptgemini视觉极限！.md", "领跑！30b模型登顶openai科研榜单，unipat-ai冲上开源科研最前线.md", "大模型拿金牌却输给三岁宝宝！一套「纯视觉考卷」把顶尖vlm打回幼儿园.md", "预测这件事，人类越犹豫，这个大模型越有优势.md", "五百行代码打造sota视觉智能体！unipat-ai最新开源.md", "科研ai出了个狠角色：开源30b小模型，硬刚gemini和claude.md", "claude-通过率不到-4%，saas-bench-撕碎了-computer-use-的「全自动办公」幻想.md", "claude不到4%，全军覆没！一场大考撕碎agent「全自动办公」幻想.md"]
---

# UniPat AI

[[UniPat AI]] 是一家 AI 研究实验室与初创公司，活跃在 AGI（通用人工智能）基础研究前沿。该团队致力于构建真实场景下 AI 训练、评测与应用的新范式，推动 Agent 能力在千行百业中规模化落地，创造切实的经济与社会价值。其核心目标包括构建 AI 预测基础设施、推动人工智能在科学研究领域的应用，以及通过极简工具集增强多模态大模型的能力。
...
### SaaS-Bench
UniPat AI 开发并发布了 [[saas-bench|SaaS-Bench]] 评测基准，这是首个在真实 SaaS 系统中全面评估 [[computer-use-agent|Computer-Use Agent]] 能力的基准测试。

- **环境构建**：将 23 个真实开源 SaaS 系统通过 Docker 本地部署，保留完整前后端逻辑和数据库状态，而非依赖仿真环境。
- **任务设计**：包含 106 个经过「LLM 生成 + 专家把关」流程构建的专业任务，覆盖软件研发、业务财务、医疗管理、团队协作、农业供应链、独立媒体六大领域。
- **评分机制**：采用双重评分——Resolved Score（严苛）和 Checkpoint Score（宽松）两个维度评估。

通过 SaaS-Bench，团队揭示了当前 Agent 在真实办公场景中的四种结构性失败模式：
- [[长程任务执行衰减]]
- [[错误级联传播]]
- [[意图-状态断层]]
- [[路径依赖不稳定性]]
...
UniPat AI 的技术理念贯穿于其预测、科研、视觉与 Agent 研究中，主要包含以下核心原则：
...
- **真实场景评测**：强调在脱离语言辅助的情况下，评估 AI 对物理世界的真实理解能力，关注构成智能地基的基础视觉技能（如精细辨别、视觉追踪、空间感知）。在 Agent 评测中，同样坚持在真实 SaaS 系统中暴露结构性缺陷，而非依赖简化模拟环境。
- **专家验证**：每个任务经过人工筛选和真实执行检查，确保评测的有效性与可靠性。
