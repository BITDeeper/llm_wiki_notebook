...
3.  **[[BlueField-4]]**：数据处理单元（DPU），负责卸载网络、存储和安全任务，并管理 AI 的上下文记忆系统。
...
6.  **Spectrum X CPO 交换机**：全球首款量产的 [[cpo-共封装光学]] 交换机。通过共封装光学技术，光学器件直接封装在芯片表面，电子信号在硅片表面直接转换为光子，消除了传统光模块和铜线中转的延迟与损耗。
...
### 4. 100% 液冷与模块化设计
- **全面液冷**：系统采用 **100% 纯液冷设计**，不仅 GPU 浸没在冷却液中，连 NVLink 交换机也完全浸泡。互联不再依赖外部线缆，而是在液冷模块内部完成板级/背板式集成。系统甚至可以使用高达 **45°C 的热水**进行冷却。这种设计不仅降低了对制冷系统的要求，还将节省下来的电力全部用于计算，显著提升了能效。
- **快速部署**：这一变革将机架安装时间从 2 天缩短至 2 小时。
