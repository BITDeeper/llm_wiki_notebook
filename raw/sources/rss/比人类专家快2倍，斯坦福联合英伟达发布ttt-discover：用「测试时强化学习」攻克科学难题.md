...
实际上，
能够让 LLM 真正进步的最直接方式是学习。
尽管「学习」和「搜索」都能随着算力扩展而良好地增长，但在 AI 的发展历史中，对于围棋、蛋白质折叠等这类困难问题，「学习」往往最终超越了「搜索」。因为，
科学发现本质是：超出训练数据与人类现有知识的 out-of-distribution 问题。
为此，
...
为了适应上述目标，其学习目标函数和搜索子程序都旨在优先考虑最有希望的解决方案 。
...
值得注意的是，该方法在使用开放模型 OpenAI gpt-oss-120b 基础上，计算成本非常低，通过使用
Thinking Machines 的
API
Tinker
，每个问题只需花费几百美元。
...
一是熵目标函数，
作用是通过指数加权来极端地偏向高奖励样本。随着 β → ∞，熵目标函数趋近于最大值（max）。然而，团队发现，在训练早期若 β 过大，会导致训练不稳定；在训练后期若 β 过小，则随着改进幅度越来越微小，优势函数会逐渐消失，这说明为不同任务设定一个统一且固定的 β 常数是非常困难的。
为此，团队为每一个初始状态自适应地设置 β(s)，通过约束由该目标函数诱导的策略的 KL 散度来实现。
二是受 PUCT 启发的状态复用策略，
采用该规则来选择初始状态。每个状态 s 的评分为：
...
可以看到，在数学领域，关于构造数学对象（如阶跃函数）来证明不等式的更紧致边界 ——Erdős 最小重叠问题任务上，之前人类最佳表现是 0.380927、AI 最佳表现 (AlphaEvolve) 是 0.380924，而 TTT-Discover 刷新记录，拿到了
0.380876
的成绩。
