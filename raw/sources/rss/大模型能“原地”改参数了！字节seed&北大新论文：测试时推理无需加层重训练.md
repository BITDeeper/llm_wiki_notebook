...
首先，
架构不兼容
。现有的TTT需要引入全新的网络层，甚至替换注意力机制，导致必须从头开始做预训练。
其次，
计算效率低
。现有的TTT采用一个Token一个Token的顺序更新，无法充分利用GPU/TPU的并行计算能力。
还有
优化目标不匹配
的问题。现有TTT多采用重建目标（reconstruction），只让模型记住当前的词，而不是为了预测下一个词设计。也就是说，与语言模型核心的“预测下一个Token”的任务不匹配。
针对这些问题，来自字节Seed和北京大学的研究团队想到了一个小妙招：
不新增层，也不改架构，直接把Transformer里本来就有的MLP模块，当成大模型的“临时小脑”。
这个名为
In-Place TTT
（原地测试时训练）的方案，让TTT可以作为
即插即用
的模块，无缝集成到现有的预训练大模型中。
...
原地架构设计
在In-Place TTT中，研究人员巧妙地
复用了Transformer中无处不在的MLP（多层感知机）
。
他们将MLP的最后一个投影矩阵W
down
作为快速权重（fast weights），在推理时进行原地更新。
...
高效的块级更新机制
In-Place TTT是对MLP进行改造，保留了原有的注意力层，这就使得该方法可以实现
分块更新
，不用再逐Token去处理。
