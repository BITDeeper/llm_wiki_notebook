...
然而，MLA的设计存在两个关键缺陷：
张量并行（TP）受阻：
由于采用单一潜在向量架构，其KV Cache无法在多个计算设备间进行切分，迫使SGLang等开源推理框架只能采用数据并行（DP）模式处理MLA解码，导致模型权重在各设备间冗余存储，无法实现高效的张量并行解码。
可扩展性与架构适配受限：
矩阵吸收后，MLA的解码在算子层面等效于一个超大维度（如576维）的MQA（Multi-Query Attention）。
