...
以Claude Sonnet 4.5为例，它在控制
最终输出
时，成功率是
61.9%。
然而，一旦查阅它的思维链，成功率瞬间跌到
2.7%
。
这种表里不一在DeepSeek R1身上达到了极致。
作为公认的强推理模型，DeepSeek R1在输出端能保持48.6%的合规率，但在思维链控制上，它的得分是惊人的
0.1%
。
在测试的13款模型中，这种落差无处不在：
GPT-5
...
不仅是训练，推理过程本身也充满了悖论。
GPT-5.2和o4-mini的数据显示：
AI
思考得越久，或者面对的题目越难，它就越顾不上表情管理。
