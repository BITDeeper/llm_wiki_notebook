...
导语
近年来，Mixture-of-Experts（MoE）已经成为大模型扩展的重要架构之一。相比稠密 Transformer，MoE 通过稀疏激活机制，在每个 token 上只调用少量专家，从而在控制计算成本的同时扩大模型容量。然而，一个长期存在的问题是：
专家
越多，并不意味着专家真的学得越 “专”。
...
针对这一问题，来自中国人民大学、华为基础模型部等机构的研究团队提出了
Mahalanobis-Pruned Mixture-of-Experts（MP-MoE）
。该工作已被
ICML 2026
接收。
...
在标准 MoE 中，路由器得到专家分数后，直接选择 top-k。这种选择可以理解：选择一个大小为
的专家子集，使其对应 score 子向量的
范数最大
。
MP-MoE 则进一步引入专家之间的相关性，构造如下目标：
其中，
表示被选专家对应的路由分数；
表示这些专家之间的某种协方差结构，目的是刻画专家之间的相似度。相比传统 MoE，MP-MoE 选择的是
马氏范数最大
的专家子集，从而在考虑专家分数的同时显式惩罚专家之间的冗余。
第二，不额外激活专家：使用专家共现矩阵刻画专家相似度。
使用马氏距离来增强多样性是自然的，但如何构造
并不自然。有一个关键问题是：
如何选取一个合适的协方差矩阵，使得在尽量不额外增加计算负担的情况下刻画专家之间的相似度？
...
被共同激活的次数。
，
是累计 token 数。作者提出用训练过程中自然产生的共现矩阵来计算专家相似度，巧妙地跳过了激活专家的步骤，大大提升了训练效率。
数值实验表明，专家共现矩阵和专家相似矩阵在结构上是
高度正相关
的，这表明该方法和通常的利用专家输出的表征来计算相似度具有一定一致性，是一种高效的替代。
左：专家输出相似度矩阵和共现矩阵的 t-SNE 图，连线代表对应专家配对。该图表明二者在结构分布上具有较高一致性。右：不同层之间专家输出和专家共现的 CKA 相似度。
这样，MP-MoE 不需要额外访问专家参数，也不需要额外激活所有专家，只需要记录路由过程中已经产生的选择模式，即专家共现矩阵
，就能动态估计专家之间的相关性。
...
为了避免每一步都重新计算矩阵逆，MP-MoE 使用
增量 Cholesky 更新
，从而显著降低计算复杂度。论文还给出了相应的理论近似保证，说明该贪心选择可以在一定条件下逼近最优专家子集。
...
在 PCA 可视化中，标准 MoE 的多个专家输出分布存在明显重叠，而 MP-MoE 中不同专家的
输出分布更加分离
。这说明 MP-MoE 训练出的专家具有更明显的功能区分。
Benchmark 结果
实验结果显示，在相同预训练预算下，MP-MoE 在多个任务上稳定优于标准 MoE 1-3 个百分点。
MP-MoE 的额外开销主要发生在训练阶段。当专家数为 64、top-k = 8 时，MP-MoE 每个 token 约额外引入
FLOPs，实际 wall-clock
预训练时间约增加 3%
。在推理阶段，MP-MoE 仍然使用与基线相同的标准 top-k 路由，因此吞吐、延迟和显存占用与 OLMoE 基线保持一致，
不引入额外推理开销
。
总结
...
第一
，它揭示了专家共现和专家相似度之间的关系，显式促进专家多样性。
第二
，它是即插即用的路由改造，不需要额外激活所有专家，也不需要改变 MoE 主体结构。
第三
，它只在训练阶段引入轻微额外开销，推理阶段可以仍然使用标准 top-k 路由，因此不会增加部署成本。
