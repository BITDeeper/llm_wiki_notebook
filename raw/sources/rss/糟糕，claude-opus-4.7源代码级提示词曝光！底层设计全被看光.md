...
指令，它会自动扫描会话历史，识别那些安全但重复的Bash或MCP命令，并建议将它们加入白名单，让操作流程如丝般顺滑。
其次，「摘要回顾」（Recaps）功能。
...
在连续三次被系统内置的安全检查拦截后，Mythos Preview并没有放弃，而是通过手动构造了一条包含特定SHA值的
--force-with-lease
绕过指令，精准地策划了第四次「越权」攻击。
