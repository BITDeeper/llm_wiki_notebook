...
刚刚发布的M3，
把前沿Coding能力、1M超长上下文、原生多模态这三条科技树，同时点满
。
...
MiniMax为此从最底层的注意力机制入手，自己做了一套新的稀疏注意力架构，叫
MSA
...
第一步是
Index Attention
，用轻量的索引query对KV块做Block Max Pool，选出Top-k的高相关块；
第二步，
再对选出来的块做完整的Sparse Attention计算
。
...
效果上，在1M上下文下，
M3每token的计算量只有上代模型的1/20
，prefilling阶段提速超过9倍，decoding阶段提速超过15倍，且绝大部分能力与全注意力持平。
多模态方面，M3
从预训练第一步就做图文混合训练
，不同模态数据的语义空间从一开始就在同一套框架下融合。
