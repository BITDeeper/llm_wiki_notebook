...
一篇是来自 Yann LeCun 团队的
LeWorldModel
，尝试以更简洁的 JEPA 实现从像素端到端训练的世界模型，在降低训练复杂度的同时，验证了潜在空间中对物理结构的刻画能力。
另一篇是清华大学团队的
Fast-WAM
，则从应用角度出发，重新审视当前主流 World Action Model（WAM）的设计范式，探讨「是否真的需要在推理阶段显式生成未来」这一关键问题，并给出了一种更高效的替代路径。
