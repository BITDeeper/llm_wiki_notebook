...
但用户清楚记得，Claude.ai的Web界面有一个明确的设计——每当模型实际调用
web_search
工具时，界面上会显示一个「已搜索网络」的指示器，旁边还有一个可点击的「>」符号，点开就能看到模型检索到的所有URL。
...
或许，差异来自Opus
4.7更严格的遵循提示词字面意思
，4.6 里被当作「可选建议」的列表，在4.7里会变成硬性要求。
