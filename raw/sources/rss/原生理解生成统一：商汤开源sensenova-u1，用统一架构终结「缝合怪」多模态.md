...
好巧，还真有。
刚刚，我们发现了一款国产开源模型，正好踩中了这个需求 —— 来自
商汤的日日新 SenseNova U1 系列原生理解生成统一模型
。
...
，也不受次数束缚。
就像这样：
SenseNova U1 不只是会「画图」，它采用了
NEO-unify 理解生成统一的全新架构，能够真正让语言和视觉信息协同
。所以它只需
8B 的小参数
，就能实现很多商业闭源模型同样的效果，效率拉满。
由于能够把视觉信息直接纳入思考链路中，它还在行业首创了连续性图文创作输出的模式。
另外，值得说明的是，商汤这次开源的是 SenseNova U1 的轻量版系列 SenseNova U1 Lite，包含
SenseNova-U1-8B-MoT
、
...
我们还用 SenseNova U1 Lite 尝试了
复杂高密度信息图（infographic）的生成
。
...
SenseNova U1 Lite 的答案，是
一套叫做 NEO-Unify 的原生多模态架构，解决这种理解和生成之间的断层
。
