...
第一类
延续传统机器学习范式
：系统先在训练集上学习，例如微调模型、优化提示词、构建记忆库或技能库，然后在测试集上评估。
第二类则
关注运行时学习
，即系统在同一个数据集上先学习多轮，然后观测其性能提升。
这些设定都具有研究价值，但真实部署还包含一个重要维度：
时间
。在真实系统中，任务是按顺序到来的。Agent 不能提前看到未来任务，也不能随意回到过去重做某个请求。每一步既是一次服务，也是一次反馈收集；当前选择不仅影响当前任务结果，也可能影响后续策略。
