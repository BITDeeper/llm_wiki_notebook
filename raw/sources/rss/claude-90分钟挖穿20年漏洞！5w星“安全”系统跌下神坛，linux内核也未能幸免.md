...
请重点检查foo.c这个文件。
然后依次下达“检查bar.c”、“检查下一个文件”指令，就能让大模型
遍历项目中的所有文件
。
根据这个方法，Anthropic披露，Claude Opus 4.6已经在开源软件库中自主识别并验证了
超500个
...
比如其中一个存在于Linux内核的NFS V4 守护进程中的漏洞，模型还
绘制出了详细的攻击流程图
，手把手解释两个恶意客户端如何通过特定数据包交互触发溢出。
而这个漏洞自2003年以来就一直存在于内核中，比
Git
还要久。
