...
Graphify
，一款零配置、全模态、本地跑、还特能省token的知识图谱工具，GitHub开源狂揽2k+ Star。
不仅能自动构建可导航知识图谱、自带反向链接和关系溯源，更实现
71.5倍token消耗节省
，直接把卡神的/raw笔记法进化到了完全体。
万物皆可图谱化
...
仅需Python 3.10及以上环境，一行命令即可完成全部部署
（PyPI包当前暂时叫 graphifyy）
：
pip install graphifyy && graphify install
如果你是在龙虾平台，可通过以下命令安装：
graphify install —platform claw
Codex用户：想用上Graphify说的并行LLM子代理提取，必须先在配置文件
~/.codex/config.toml
的
[features]
里打开
