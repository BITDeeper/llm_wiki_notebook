...
最新开源
CUDA Agent
，在GPU内核优化基准KernelBench上拿下当前最优成绩：
...
第一步是
种子算子挖掘
，来源是torch和transformers，每个算子被表示为一个包含初始化方法和前向方法的Python类。
第二步是
组合合成
，随机采样最多5个torch算子，按顺序拼接成融合任务。第三步是执行驱动过滤，只保留在eager模式和compile模式下都能正常运行的任务，同时去除含有随机算子的样本。
