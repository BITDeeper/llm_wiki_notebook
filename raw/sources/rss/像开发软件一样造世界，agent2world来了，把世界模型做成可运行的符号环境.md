...
让模型真正 “
能行动
”，往往需要一个可执行、可验证的符号世界模型（Symbolic World Model）：它不是抽象的文字描述，而是能被规划器或执行器直接调用的形式化定义 —— 例如 PDDL 领域 / 问题，或可运行的环境代码 / 模拟器。一旦世界被 “写成可运行的规则”，我们就能在同一套约束下进行推演、测试与复现：模型不再停留在 “会说”，而是能回答 “如果我这样做，会发生什么”，并用执行结果检验自己是否真的理解了这个世界。
...
但当移除unit tester后，在离散动作空间的预测准确率显著下降约 30%。移除simulation tester，也会同比下降约3%
。这揭示了一个关键发现：“能运行” 不等于 “物理正确”。没有动态交互产生的行为级反馈，模型很难在该设置下修正深层的动力学错误（如重力模拟偏差），生成的模拟器也因此失去了实用价值。
